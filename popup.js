...
const defaults = Object.assign({}, Base.DEFAULTS, {
  chatModel: Base.DEFAULT_TRANSLATION_MODEL,
  sendModel: Base.DEFAULT_GEMINI_MODEL,
  speechModel: Base.DEFAULT_TRANSLATION_MODEL,
  sendEnabled: false,
...
function populateModelSelects() {
  for (const id of ["chatModel", "sendModel", "speechModel"]) {
    ...
    const models = id === "sendModel" ? Base.GEMINI_MODELS : Base.TRANSLATION_MODELS;
...
function syncBatchChunkVisibility() {
  const chatModel = document.getElementById("chatModel").value;
  const entry = Base.TRANSLATION_MODELS.find(m => m.value === chatModel);
  const group = document.getElementById("batchChunkGroup");
  if (group) group.style.display = entry && entry.provider === "gemini" ? "" : "none";
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  chrome.storage.local.get(defaults, (cfg) => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }).then(([tab]) => {
      const url = tab?.url || "";
      const isYouTube = url.startsWith(UbyeBase.YOUTUBE_ORIGIN + "/watch") || url.startsWith(UbyeBase.YOUTUBE_ORIGIN + "/shorts/") || url.startsWith(UbyeBase.YOUTUBE_ORIGIN + "/live/");
  ...
  document.getElementById("chatModel").addEventListener("change", syncBatchChunkVisibility);
