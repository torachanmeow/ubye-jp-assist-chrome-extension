...
document.addEventListener("DOMContentLoaded", () => {
  ...
  chrome.storage.local.get(defaults, (cfg) => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, ([tab]) => {
      const url = tab?.url || "";
      const isVideoPage = /youtube\.com\/(watch|shorts\/)/.test(url);
      const startBtn = document.getElementById("sttStart");
      if (!isVideoPage) {
        startBtn.disabled = true;
        document.getElementById("pageNotice").style.display = "";
      }
      chrome.runtime.sendMessage({ type: MSG.STT_GET_CAPTURE_TAB }, (captureTabId) => {
  ...
  document.getElementById("sttStart").addEventListener("click", async () => {
    ...
    try {
      ...
      if (res?.ok) {
        ...
        if (res?.error === "別タブで使用中です") keepDisabled = true;
      }
    } catch (e) {
  ...
  document.getElementById("save").addEventListener("click", () => {
    ...
    if (geminiApiKey && !/^AIza[0-9A-Za-z_-]{35,}$/.test(geminiApiKey)) {
