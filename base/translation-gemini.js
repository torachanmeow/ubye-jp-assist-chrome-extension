...
(function (Base) {
  ...
  const RETRY_BASE_DELAY_MS = 800; // バックオフの基準値（attempt 0 の待ち時間）
  const MAX_RETRIES = 3;           // 503/429 は数秒継続することがあるため複数回リトライする

  async function callGeminiApi(apiKey, model, systemPrompt, text, maxTokens, attempt = 0) {
    ...
    if (!res.ok) {
      if (attempt < MAX_RETRIES && RETRY_STATUSES.has(res.status)) {
        // Retry-After があれば尊重し、なければ指数バックオフ。
        // 同時失敗した複数リクエストが一斉に再送して再び過負荷になるのをジッタで散らす。
        const retryAfter = parseInt(res.headers.get("retry-after"), 10);
        const backoff = Number.isFinite(retryAfter)
          ? retryAfter * 1000
          : RETRY_BASE_DELAY_MS * 2 ** attempt;
        const delay = backoff + Math.floor(Math.random() * 300);
        log.debug("リトライ:", res.status, `(${attempt + 1}/${MAX_RETRIES}) ${delay}ms`);
        await new Promise((r) => setTimeout(r, delay));
        return callGeminiApi(apiKey, model, systemPrompt, text, maxTokens, attempt + 1);
      }
