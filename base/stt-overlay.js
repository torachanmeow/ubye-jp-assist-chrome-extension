...
(function (Base) {
  if (Base.sttOverlay) return; // 動的注入の冪等性ガード
  const MSG = Base.MSG;
  ...
  /** init 側から注入される状態アクセサ */
  let getState = () => ({ apiKeyConfigured: false, speechAutoTranslate: false });
  let toggleAutoTranslate = () => {};

  function needsTranslation() {
    return Base.sttLang !== "ja-JP" && Base.isTranslationAvailable(getSpeechModel(), getState().apiKeyConfigured);
  }
  ...
  function syncAutoTranslateBtn() {
    ...
    btn.style.display = "";
    btn.classList.toggle("ubye-stt-btn-active", getState().speechAutoTranslate);
    btn.title = "自動翻訳 ON/OFF";
  }

  const getSpeechModel = Base.createStorageValue("speechModel", Base.DEFAULT_TRANSLATION_MODEL, () => syncTitle());

  function shortModelName(value) {
    if (value === Base.CHROME_BUILTIN_MODEL) return "内蔵";
    return value.replace(/^gemini-/, "").replace(/-preview$/, "");
  }

  function buildTitle() {
    const model = getSpeechModel() && Base.sttLang !== "ja-JP" ? " / " + shortModelName(getSpeechModel()) : "";
  ...
  function requestSttTranslation(text, line, updateSubtitle) {
    return Base.translate.speech(text).then((res) => {
  ...
  function handleFinal(msg) {
    ...
    const status = translate ? (state.speechAutoTranslate ? "translating" : "idle") : "no-translate";
    ...
    if (translate && state.speechAutoTranslate) {
      requestSttTranslation(displayText, line, true);
      return id;
    }
  }
  ...
  Base.sttOverlay = {
    ...
    /**
     * init 側から状態アクセサを注入する。
     * @param {function} stateFn - () => { apiKeyConfigured, speechAutoTranslate } を返す関数
     * @param {function} toggleFn - 自動翻訳トグル時のコールバック
     */
    bind(stateFn, toggleFn) {
      getState = stateFn;
      toggleAutoTranslate = toggleFn;
    },
  };
})(UbyeBase);
