...
(function (Base) {
  ...
  let menusReady = getMenuState().then((state) => createMenus(state)).catch((e) => {
    log.warn("メニュー作成失敗:", e.message);
  });
  ...
  chrome.contextMenus.onClicked.addListener(async (info, tab) => {
    ...
    if (info.menuItemId === MENU_ID.AUTO_TRANSLATE) {
      ...
      if (!state.speechAutoTranslate) {
        const cfg = await chrome.storage.local.get({ geminiApiKey: "", speechModel: Base.DEFAULT_TRANSLATION_MODEL });
        if (!Base.isTranslationAvailable(cfg.speechModel, !!cfg.geminiApiKey)) {
          chrome.tabs.sendMessage(tab.id, {
            type: Base.MSG.TOAST,
            text: "API Keyが未設定です",
