...
(function (App, Base) {
  ...
  const langDetect = Base.createLangDetector({
    kanaRange: Base.SKIP_LANG.jaKanaRange,
    charRange: Base.SKIP_LANG.jaCharRange,
    ratio: Base.SKIP_LANG.jaRatio,
  });
  ...
  async function translateBatch(texts) {
    const res = await sendMsg({ type: MSG.TRANSLATE_BATCH, texts });
    return res.results;
  }
        ...
        try {
          ...
          for (let i = 0; i < chunk.length; i++) {
            ...
            if (!translated || translated.trim() === text.trim()) {
              // 翻訳失敗（元テキストと同一）→ translatedSet に入れず次サイクルで再試行
    ...
    chatAutoCountdown = setInterval(() => {
      ...
      if (remaining <= 0) {
        translateVisibleMessages("chat").then(() => {
          if (App.state.chatAutoEnabled) startChatAutoCountdown();
        });
      }
  ...
  function setupTranslateVisibleBtn(doc) {
    if (!App.state.chatTranslateEnabled || !App.state.apiKeyConfigured) {
      App.state.chatAutoEnabled = false;
      stopChatAutoCountdown();
  ...
  function setupPollTranslateBtn() {
    if (Base.sttLang === "ja-JP" || !App.state.apiKeyConfigured) return;
