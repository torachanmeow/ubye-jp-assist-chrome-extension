...
(function (App, Base) {
  ...
  function translateSend(text) {
    return sendMsg({ type: MSG.TRANSLATE_SEND, text }).then((res) => {
      if (!res?.ok) throw new Error(res?.error || "翻訳に失敗しました");
    ...
    const onClick = async () => {
      ...
      try {
        ...
        if (originalText !== text) {
          // await 中にユーザーが入力して originalText が変化した場合は入力欄に触れない
        ...
        originalText = null;
        ignoreNextInput = false;
        Base.showToast(e.message || "翻訳に失敗しました");
      } finally {
        btn.disabled = false;
        btn.textContent = Base.sendLabel;
      }
