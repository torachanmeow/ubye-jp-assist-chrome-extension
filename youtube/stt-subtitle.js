...
(function (App, Base) {
  ...
  function hasYouTubeCaptions() {
    const btn = document.querySelector(".ytp-subtitles-button[aria-pressed='true']");
    if (btn) return true;
    const captionWin = document.querySelector(".caption-window");
    return captionWin && captionWin.offsetHeight > 0;
  }

  function adjustPosition() {
    if (!containerEl) return;
    containerEl.classList.toggle("ubye-subtitle-shifted", hasYouTubeCaptions());
  }
  ...
  function create() {
    ...
    captionObserver = new MutationObserver(() => {
      ...
      adjustTimer = setTimeout(() => { adjustTimer = null; adjustPosition(); }, 200);
    });
    captionObserver.observe(player, { childList: true, subtree: true });
    const subBtn = player.querySelector(".ytp-subtitles-button");
    if (subBtn) captionObserver.observe(subBtn, { attributes: true, attributeFilter: ["aria-pressed"] });
  }
