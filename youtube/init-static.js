...
App.state.chatModel = Base.DEFAULT_TRANSLATION_MODEL;
...
const storageHandlers = {
  ...
  geminiApiKey(v) { App.state.apiKeyConfigured = !!v; App.chat.resetTranslations(); },
  chatModel(v) { App.state.chatModel = v; App.chat.resetTranslations(); },
  profile(v) {
    Base.applyProfile(v);
    App.chat.resetTranslations();
...
chrome.storage.local.get({ sendEnabled: false, chatTranslateEnabled: false, geminiApiKey: "", chatModel: Base.DEFAULT_TRANSLATION_MODEL, profile: Base.DEFAULT_PROFILE }, (cfg) => {
  ...
  App.state.chatModel = cfg.chatModel;
...
function checkPageChange() {
  ...
  const wasVideo = lastUrl.startsWith("/watch") || lastUrl.startsWith("/shorts/") || lastUrl.startsWith("/live/");
  const isVideo = currentUrl.startsWith("/watch") || currentUrl.startsWith("/shorts/") || currentUrl.startsWith("/live/");
  lastUrl = currentUrl;
  if (!wasVideo) return;
